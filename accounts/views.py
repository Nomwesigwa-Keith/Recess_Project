...
@login_required
def technician_profile_settings(request):
    user = request.user
    if request.method == 'POST':
        form = AdminProfileForm(request.POST, instance=user)
        if form.is_valid():
            form.save()
            messages.success(request, 'Profile updated successfully.')
            return redirect('technician_profile_settings')
    else:
        form = AdminProfileForm(instance=user)
    return render(request, 'admin_profile_settings.html', {'form': form})

@login_required
def farmer_profile_settings(request):
    user = request.user
    if request.method == 'POST':
        form = AdminProfileForm(request.POST, instance=user)
        if form.is_valid():
            form.save()
            messages.success(request, 'Profile updated successfully.')
            return redirect('farmer_profile_settings')
    else:
        form = AdminProfileForm(instance=user)
    return render(request, 'admin_profile_settings.html', {'form': form})
